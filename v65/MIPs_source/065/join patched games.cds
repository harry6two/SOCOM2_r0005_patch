...
// check if password exists -- doing this avoids TLB miss or freeze
lbu t0, $0000(a1)
beq t0, zero, :PATCH__normal_game
nop


// ----------------- check for password patch game
addiu t1, zero, $23 // "#" character
// check if first letter is "#"
beq t0, t1, :__legacyPATCHgame_continue
nop

// ----------------- check for password sgl game
addiu t1, zero, $25 // "%" character
beq t0, t1, :__sglPATCHgame_continue
nop


// ----------------- check for password legacy patch game
addiu t1, zero, $24 // "$" character
// check if first letter is "$"
bne t0, t1, :__checkForPatchPassword
nop
// check if 2nd char is patch char
lbu t2, $0001(a1)
addiu t3, zero, $F1
beq t2, t3, :__legacyPATCHgame
nop
// else not patch game
beq zero, zero, :PATCH__normal_game
nop

// ----------------- check for patched game
__checkForPatchPassword:
...
__legacyPATCHgame_continue:
jal :__LEGACY_PATCHEDGAME
nop
jal :__disable_player_dynamics
nop
beq zero, zero :__end
...
__sglPATCHgame_continue:
jal :__SGL_PATCHEDGAME
nop
jal :__disable_player_dynamics
nop
beq zero, zero :__end
...
__join_game_patchPASSWORD_check_FNC:
addiu sp, sp, $fff0
sw ra, $0000(sp)
sw a0, $0004(sp)

// a0 = password text pointer
// -- check if password is for patch game by checking 1st char for "#"
lbu t0, $0000(a0)
addiu t1, zero, $23 // "#" character
// check if first letter is "#"
beq t0, t1 :__continue_to_passScamble
nop
// check if SGL game
addiu t1, zero, $25 // "%" character
// check if first letter is "#"
bne t0, t1 :__not_patchgamePASSWORD
nop

__continue_to_passScamble:
// encode rest of password
jal :__password_scramble_PASSWORDpatch
// increment to rest of password
addiu a0, a0, $1


__not_patchgamePASSWORD:
// original code
lw ra, $0000(sp)
lw a0, $0004(sp)
j $003A59C0
...
__password_scramble_PASSWORDpatch:
// copy password ptr 
daddu t0, a0, zero
// set xor key
addiu t1, zero, $e4
__get_pw_byte_PASSWORD:
// get password byte
lb t2, $0000(t0)
beq t2, zero, :__end_sramble_PASSWORD
nop
...
beq zero, zero, :__get_pw_byte_PASSWORD
addiu t0, t0, $1 //increment to next byte
__end_sramble_PASSWORD:
jr ra
nop
...
// PATCH GAME PASSWORD - check first character in password "#"
addiu t0, zero, $23 // "#"
lbu t1, $0000(s4) // get first character of password
beq t1, t0 :__is_PASSWORDpatched_game
nop

// SGL GAME PASSWORD - check first character in password "%"
addiu t0, zero, $25
lbu t1, $0000(s4) 
beq t1, t0 :__is_SGL_PASSWORD_game
nop

// PATCH GAME - check first character in password "!"
addiu t0, zero, $21 // "!"
beq t1, t0 :__is_patched_game
nop

// check if legacy patch game
addiu t0, zero, $24 // "$"
beq t1, t0 :__is_legacyPatchGame
nop

// HALO GAME - check first character in password "@"
addiu t0, zero, $40 // "@"
//lbu t1, $0000(s4) // get first character of password
...
__is_SGL_PASSWORD_game:
// GAME IS PATCHED with PASSWORD
// - 2nd letter in password is not altered for game join function

// scramble rest of password
addiu a0, s4, $1 // move password past first position
jal :__password_scramble_PASSWORDpatch
nop
jal :__SGL_PATCHEDGAME
nop
...
sw t1, $0000(t0)
// set patched game code stack
setreg a0, :__sgl_patchgame_codes
jal :__code_decryption_fnc
nop
...
// PATCH CODE
// decrypted values stored in change.txt
__patchgame_codes:
